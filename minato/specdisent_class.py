...
class spectral_disentangling:
    def __init__(self, lines, orbparams, epochs, spectra, extension='.fits'):
        ...
        lines_dic = {
                        ...
                        4026: { 'region':[4000., 4050.], 'title':'He I $\lambda$4026'},
                        4102: { 'region':[4070., 4140.], 'title':'H$\delta$'},
                        4121: { 'region':[4105., 4136.], 'title':'He I $\lambda$4121, Si II $\lambda$4128/32'},
                        4144: { 'region':[4120., 4170.], 'title':'He I $\lambda$4144'},
                        4233: { 'region':[4225, 4241], 'title':'Fe II $\lambda$4233'},
                        4267: { 'region':[4260, 4275], 'title':'C II $\lambda$4267'},
                        4340: { 'region':[4310., 4370.], 'title':'H$\gamma$'},
                        4388: { 'region':[4365., 4410.], 'title':'He I $\lambda$4388'},
                        4471: { 'region':[4455., 4495.], 'title':'He I $\lambda$4471, Mg II $\lambda$4481'},
                        # 4553: { 'region':[4536, 4560], 'title':'Fe II $\lambda$4550/56, Si III $\lambda$4553'} }
                        4553: { 'region':[4545., 4580.], 'title':'He II $\lambda$4542, Si III $\lambda$4553'}, 
        ...
        df = pd.read_csv(orbparams, skipinitialspace=True, header=0)
        # display(df.columns)
        ...
        # Set working path
        working_path = spectra+'disentangling_'+str(date.today())+'/'
    ...
    def get_disspec(self, lguess1=0.7, S2Nblue=4200, S2Nred=4215, GridDis=True, oneD=False, Triple=False, CleanCos=False, PLOTCONV=False, itrnumlim=30, NumItrFinal=500,
                    PLOTITR=False, N_Itr_Plot=5, PLOTFITS=False, PLOTEXTREMES=True, NebOff=False, rmNebChi2=False, StrictNegA=True, StrictNegB=True, ContSig=3, Renormalise=False, NormPoints=None, 
        ...
        PosLimCondC = np.array([ 
                [3965, 3975], [4096., 4106.], [4335, 4345], [4466, 4476], [4856, 4866]
                ])
        ...
        fig, ax = plt.subplots()
        plt.plot(wavegridall, A, label='dis')
        plt.plot(wavegridall, B, label='dis')
        if not NebOff:
    ...
    def CalcDiffs(self, A, B, C, vrA, vrB, waves, Resid=False, Reduce=False, ShowItr=False, NebFac=1., linewidExt=3, legsize=9, locleg='lower left', alphaleg = 0.):
        ...
        for ind in np.arange(len(self.ObsSpecs)):
            ...
            # User needs to change kcount==1 condition if a specific K2 is desired for plotting.             
                if kcount==kcount_usr:      
                    plt.plot(waves[WaveCalcCond], specsum)
                    plt.plot(waves[WaveCalcCond], Ashift)
                    plt.plot(waves[WaveCalcCond], Bshift)   
                    plt.plot(waves[WaveCalcCond], Cshift)                  
                    plt.plot(waves[WaveCalcCond], ObsSpec, label=self.specnamesFin[ind].split('/')[-1]  + r', $\varphi=$' + str(round(self.phis[ind], 2)))
                    plt.title('PLOTFITS')
                    plt.legend()
            ...
            if self.PLOTEXTREMES:
                if self.kcount==self.kcount_extremeplot:
                    if ind==min(self.RVExtminInd, self.RVExtmaxInd):
                        ...
                        axes[0].plot(waves[WaveCalcCond], Ashift, label='Prim Dis.', color='red', linestyle = 'dotted', linewidth=linewidExt)
                        axes[0].plot(waves[WaveCalcCond], Bshift, label='Sec. Dis.', color='green')   
                        axes[0].plot(waves[WaveCalcCond], Cshift, label='Nebular', color='purple') 
                        axes[0].plot(waves[WaveCalcCond], ObsSpec, color='blue', label=str(round(self.MJDs[ind],0)) + r', $\varphi=$' + str(round(self.phis[ind], 2)))
                        axes[0].plot(waves[WaveCalcCond], specsum, label='Sum Dis.', color='black', linestyle = '--', linewidth=linewidExt)                    
                        axes[0].set_title(self.specnamesFin[ind].split('/')[-1])
                        ...
                        axes[0].yaxis.set_minor_locator(MultipleLocator(.01))
                    elif ind==max(self.RVExtminInd, self.RVExtmaxInd):
                        axes[1].plot(waves[WaveCalcCond], Ashift, color='red', linestyle = 'dotted', linewidth=linewidExt)
                        axes[1].plot(waves[WaveCalcCond], Bshift, color='green')   
                        axes[1].plot(waves[WaveCalcCond], Cshift,  color='purple')
                        axes[1].plot(waves[WaveCalcCond], ObsSpec, color='blue', label=str(round(self.MJDs[ind],0)) + r', $\varphi=$' + str(round(self.phis[ind], 2)))                    
                        axes[1].plot(waves[WaveCalcCond], specsum, color='black', linestyle = '--', linewidth=linewidExt)                    
                        axes[1].set_title(self.specnamesFin[ind].split('/')[-1])                    
                        ...
                        axes[1].yaxis.set_minor_locator(MultipleLocator(.01))                       
                        plt.tight_layout()    
    ...
    def Chi2con(self, redchi2, nu, P1=0.68, comp='secondary', fitrange = 6):    
        if comp == 'secondary':
